...
class Hyperparameters:
    ...
    gptq_damp_ratio = float(os.environ.get("GPTQ_DAMP_RATIO", 0.01))
    # AR self-gen calibration knobs
    calib_num_seqs = int(os.environ.get("CALIB_NUM_SEQS", 64))
    calib_seq_len = int(os.environ.get("CALIB_SEQ_LEN", 2048))
    calib_temperature = float(os.environ.get("CALIB_TEMPERATURE", 0.8))
    calib_batch_size = int(os.environ.get("CALIB_BATCH_SIZE", 8))
...
def generate_autoregressive_calib(model, device, num_seqs=64, seq_len=2048,
                                   ...
                                   top_p=0.0, top_k=0, prompt_ids=None):
    """Generate sequences autoregressively from the model for GPTQ calibration.
    No external data accessed — fully self-contained.
    top_p > 0 enables nucleus sampling; top_k > 0 enables top-k filtering.
    prompt_ids: optional 1-D tensor of token IDs to prefix every sequence."""
    model.eval()
    rng = torch.Generator(device=device)
    rng.manual_seed(seed)
    all_tokens = []
    prompt_len = 0 if prompt_ids is None else prompt_ids.numel()
    with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        for batch_start in range(0, num_seqs, batch_size):
            ...
            if prompt_ids is not None and prompt_ids.numel() > 0:
                tokens = prompt_ids.unsqueeze(0).expand(bs, -1).to(device)
            else:
                tokens = torch.randint(0, vocab_size, (bs, 1), device=device, generator=rng)
            for pos in range(seq_len - tokens.shape[1]):
                ...
                if top_p > 0.0:
                    ...
                    next_logit = torch.zeros_like(next_logit).scatter(1, sorted_indices, sorted_logits)
...
def collect_hessians_from_tokens(hessian_model, token_seqs, device, damp_ratio=0.01):
...
def quantize_int6_gptq(weight, hessian=None, clip_range=31, block_size=128, damp_ratio=0.01):
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hessians: dict[str, Tensor] | None = None, gptq_block_size: int = 128, gptq_damp_ratio: float = 0.01):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            if H is not None:
                q, s = quantize_int6_gptq(t, hessian=H, clip_range=cr, block_size=gptq_block_size, damp_ratio=gptq_damp_ratio)
            else:
